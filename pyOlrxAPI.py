...
def InitOlrxAPI(dllPath):
    ...
    Args:
        dllPath (string):
            - Full path name of ASPEN program folder where related
              program components are located
              dllFile (default) = "olrxapi.dll"
            or
            - Full path and file name of dllFile
    Returns:
        None
    ...
    """
    #
    if str(dllPath).endswith(".dll"):
        dllPath1 = os.path.dirname (os.path.abspath(dllPath)) + os.sep
        dllFile = os.path.basename(os.path.abspath(dllPath))
    else:
        dllPath1 = os.path.abspath(dllPath) + os.sep
        dllFile = "olrxapi.dll"

    # get the module handle and create a ctypes library object
    if not os.path.isfile(dllPath1 + dllFile) :
        raise OlrxAPIException("Failed to locate: " + dllPath1 + dllFile)
    os.environ['PATH'] = os.path.abspath(dllPath1) + ";" + os.environ['PATH']
    global ASPENOlrx
    ASPENOlrx = WinDLL((dllPath1 + dllFile).encode('UTF-8'), use_last_error=True)
    if ASPENOlrx == None:
        raise OlrxAPIException("Failed to setup "+dllFile)
    ...
    #
    print ("ASPEN OlrxAPI path: ", dllPath1, "\n\tVersion:", OlrxAPIVersion(), " Build: ", OlrxAPIBuildNumber())

#
def OlrxAPIGetEquipementData(ehnd,paraCode,VT_type):
    """
   ...
   """
    ...
    if VT_type ==VT_INTEGER:
        ...
        raise OlrxAPIException("Error of VT_type")
    # get data
    res = []
    for paraCode1 in paraCode:
        r1 = []
        for ehnd1 in ehnd:
            if ( OLRXAPI_FAILURE == OlrxAPIGetData( ehnd1, c_int(paraCode1), byref(val1) ) ) :
...
def OlrxAPILoadDataFile(filePath,readonly):
    ...
    if OLRXAPI_OK == ASPENOlrx.OlrxAPILoadDataFile(filePath,readonly):
