...
def getAllBranchesAndBus():
   ...
   """
    ...
    # TC_SCAP
    hndBr = c_int(0)
    while ( OLRXAPI_OK == OlrxAPIGetEquipment(c_int(TC_SCAP), byref(hndBr) )) :
        bus.append(OlrxAPIGetBusByBranch(hndBr,TC_SCAP))
        br.append(hndBr.value)
    # TC_PS
    hndBr = c_int(0)
    while ( OLRXAPI_OK == OlrxAPIGetEquipment(c_int(TC_PS), byref(hndBr) )) :
        bus.append(getBusByBranch(hndBr,TC_PS))
        br.append(hndBr.value)
    # TC_SWITCH
    hndBr = c_int(0)
    while ( OLRXAPI_OK == OlrxAPIGetEquipment(c_int(TC_SWITCH), byref(hndBr) )) :
        bus.append(getBusByBranch(hndBr,TC_SWITCH))
        br.append(hndBr.value)
    # TC_XFMR
    hndBr = c_int(0)
    while ( OLRXAPI_OK == OlrxAPIGetEquipment(c_int(TC_XFMR), byref(hndBr) )) :
        bus.append(getBusByBranch(hndBr,TC_XFMR))
        br.append(hndBr.value)
    # TC_XFMR3
    hndBr = c_int(0)
    while ( OLRXAPI_OK == OlrxAPIGetEquipment(c_int(TC_XFMR3), byref(hndBr) )) :
...
def getBusByBranch(hndBr,TC_Type):
    ...
    hndBus1 = c_int(0)
    for b1 in busHnd:
        if ( OLRXAPI_FAILURE == OlrxAPIGetData( hndBr, c_int(b1), byref(hndBus1) ) ) :
            raise OlrxAPIException(OlrxAPIErrorString())
        #
